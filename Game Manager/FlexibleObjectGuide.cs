...
public class FlexibleObjectGuide : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private float hysteresisBuffer = 0.5f; // Buffer to prevent jitter
    ...
    [SerializeField]
    private float screenPadding = 70f; // Padding from screen edges
    ...
    [SerializeField]
    private Image arrowImagePrefab; // Required prefab for the arrow Image
    
    [SerializeField]
    private TextMeshProUGUI distanceTextPrefab; // Optional prefab for distance text
    
    [SerializeField]
    private TextMeshProUGUI nameTextPrefab; // Optional prefab for name text
    
    [Header("Offsets")]
    [SerializeField]
    private Vector2 arrowOffset = new Vector2(0, 0f); // Offset for arrow
    
    [SerializeField]
    private Vector2 distanceOffset = new Vector2(0, -20f); // Offset for distance
    
    [SerializeField]
    private Vector2 nameOffset = new Vector2(0, -20f); // Offset for name
    ...
    private void CheckForObjects()
    {
        ...
        GameObject[] allObjects = string.IsNullOrEmpty(targetTag) ? Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None) : GameObject.FindGameObjectsWithTag(targetTag);
    ...
    {
        if (uiCanvas == null || arrowImagePrefab == null)
        {
            Debug.LogError("Missing required UI Canvas or arrow prefab!");
            return;
        }
        ...
        {
            distanceText = Instantiate(distanceTextPrefab, uiCanvas);
        ...
        {
            nameText = Instantiate(nameTextPrefab, uiCanvas);
        ...
        if (nameText != null)
        {
            ...
            nameText.text = info != null && !string.IsNullOrEmpty(info.objectName) ? info.objectName : (string.IsNullOrEmpty(cleanName) ? defaultName : cleanName);
        }
    ...
    private void UpdateIndicators()
    {
        foreach (var pair in indicators)
        {
            ...
            if (obj == null || !obj.activeInHierarchy || arrow == null) continue;
            ...
            if (distanceText != null)
            {
                ...
                distanceText.transform.position = Vector3.Lerp(distanceText.transform.position, clampedScreenPos + (Vector3)distanceOffset, smoothSpeed * Time.deltaTime);
            }

            if (nameText != null)
            {
                nameText.gameObject.SetActive(true);
                nameText.transform.position = Vector3.Lerp(nameText.transform.position, clampedScreenPos + (Vector3)nameOffset, smoothSpeed * Time.deltaTime);
            }
        }
    }
